...
fn main() {
    ...
    write!(out, "{}", solve(&arr, true).min(solve(&arr, false))).unwrap();
}

fn solve(arr: &Vec<Vec<char>>, is_row: bool) -> i64 {
    let mut count = vec![[0i64; 26]; 3];

    for t in 0..3 {
        if is_row {
            for i in t * 2..t * 2 + 2 {
                for j in 0..arr[i].len() {
                    count[t][(arr[i][j] as u8 - b'A') as usize] += 1;
                }
            }
        } else {
            for i in 0..arr.len() {
    ...
    for c0 in 0..26 {
        for c1 in 0..26 {
            ...
            for c2 in 0..26 {
                ...
                result =
                    result.min((18 - count[0][c0]) + (18 - count[1][c1]) + (18 - count[2][c2]));
            }
        }
    }
