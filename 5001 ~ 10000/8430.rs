...
fn main() {
    ...
    let (mut now, mut answer) = (0, 0);
    let mut pq = BinaryHeap::new();
    for (arrival, b) in arr {
        while now < arrival {
            if let Some(Reverse((mut num, start))) = pq.pop() {
                let dif = arrival - now;
        ...
        pq.push(Reverse((b, arrival)));
    }

    while let Some(Reverse((num, start))) = pq.pop() {
        now += num;
        answer += now - start;
    }
