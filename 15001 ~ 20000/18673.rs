...
fn main() {
    ...
    for _ in 0..scan.token::<usize>() {
        ...
        let (mut gcd_num, mut gcd_den) = (gcd(a, c), lcm(b, d));

        let g3 = gcd(gcd_num, gcd_den);
        gcd_num /= g3;
        gcd_den /= g3;

        let (mut lcm_num, mut lcm_den) = (lcm(a, c), gcd(b, d));

        let g4 = gcd(lcm_num, lcm_den);
        lcm_num /= g4;
        lcm_den /= g4;

        writeln!(out, "{}/{} {}/{}", gcd_num, gcd_den, lcm_num, lcm_den).unwrap();
    }
}
