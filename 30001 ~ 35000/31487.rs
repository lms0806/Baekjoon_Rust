...
fn main() {
    ...
    for i in 0..scan.token::<usize>() {
        let (name, p) = (scan.token::<String>(), scan.token::<i64>());
        let mut temp = (0..6).map(|_| scan.token::<i64>()).collect::<Vec<_>>();

        temp.sort_unstable();

        arr.push((i, name, p * 10 + temp[1..5].iter().sum::<i64>()));
    }

    arr.sort_unstable_by(|a, b| b.2.cmp(&a.2).then(a.0.cmp(&b.0)));

    let (mut max, mut prev, mut count) = (0, -1, 0);
    for (_, name, score) in arr {
        if score != prev {
            max = count;
            prev = score;
        }
