...
fn main() {
    ...
    for _ in 0..n {
        ...
        answer += calc_damage(e, r, a, d) - calc_damage(s, r, a, d);
    }
    writeln!(out, "{:.6}", answer / (e - s)).unwrap();
}

fn calc_damage(time: f64, r: f64, a: f64, d: f64) -> f64 {
    let cycle = r + a;
    let count = (time / cycle).floor();

    let rem = time % cycle;

    if rem > r {
        return count * d + (rem - r) * (d / a);
    }
    count * d
}
