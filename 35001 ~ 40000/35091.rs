...
fn main() {
    ...
    for _ in 0..m {
        arr[scan.token::<usize>()].push(scan.token::<usize>());
    }

    let (a, b) = (scan.token::<usize>(), scan.token::<usize>());

    let (mut visited_a, mut visited_b) = (vec![false; n + 1], vec![false; n + 1]);

    bfs(a, &arr, &mut visited_a);
    bfs(b, &arr, &mut visited_b);
...
fn bfs(start: usize, arr: &[Vec<usize>], visited: &mut Vec<bool>) {
