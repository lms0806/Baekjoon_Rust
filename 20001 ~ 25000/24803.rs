...
fn main() {
    ...
    let num = 3 * scan.token::<i64>() + 2 * scan.token::<i64>() + scan.token::<i64>();

    let victory = if num >= 8 {
        Some("Province")
    } else if num >= 5 {
        Some("Duchy")
    } else if num >= 2 {
    ...
    if let Some(v) = victory {
        write!(out, "{} or {}", v, treasure).unwrap();
    } else {
        write!(out, "{}", treasure).unwrap();
    }
}
