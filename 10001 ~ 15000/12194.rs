...
fn main() {
    ...
    for i in 1..=scan.token::<usize>() {
        ...
        let result = check(target, &arr);

        let mut answer = if result == -1 { i64::MAX } else { result + 1 };

        for i in 2..=target {
            let d = check(i, &arr);
            if d != -1 {
                dfs(i, d, &arr, target, &mut answer);
            }
        }
...
fn dfs(sum: i64, count: i64, arr: &[i64; 10], target: i64, answer: &mut i64) {
    ...
    for i in 2..=target / sum {
        ...
        let result = check(i, arr);
        if result != -1 {
            dfs(sum * i, count + result + 1, arr, target, answer);
        }
    }
}

fn check(mut num: i64, arr: &[i64; 10]) -> i64 {
    if num == 0 {
