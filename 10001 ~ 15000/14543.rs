...
fn main() {
    ...
    for t in 1..=scan.token::<usize>() {
        let s = scan.line();
        let arr = s.split_ascii_whitespace().collect::<Vec<_>>();

        let (a, b, c) = (
            arr[0][..arr[0].len() - 1].parse::<f64>().unwrap(),
            arr[2].parse::<f64>().unwrap(),
            arr[4].parse::<f64>().unwrap(),
        );

        writeln!(out, "Equation {}", t).unwrap();
        ...
        if a == 0.0 {
            ...
            writeln!(out, "x = {:.6}", (x * 1_000_000.0).trunc() / 1_000_000.0).unwrap();
        }
        writeln!(out).unwrap();
    }
}
