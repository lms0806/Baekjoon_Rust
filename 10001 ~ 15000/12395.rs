...
fn main() {
    ...
    for i in 1..=scan.token::<usize>() {
        ...
        let mut arr = vec![1.0; a + 1];
        for i in 1..=a {
            arr[i] = arr[i - 1] * scan.token::<f64>();
        }

        let mut answer = (b + 2) as f64;

        for k in 0..=a {
            answer = answer
                .min(k as f64 + (b - (a - k)) as f64 + 1.0 + (1.0 - arr[a - k]) * (b as f64 + 1.0));
        }

        writeln!(out, "Case #{}: {:.6}", i, answer).unwrap();
    }
}
